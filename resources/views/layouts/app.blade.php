    ...
    <script>
        ...
        function detectContentType(url) {
            if (/instagram\.com\/(reel|reels)\//i.test(url)) {
                return { type: 'reels', label: 'Instagram Reels detected', color: 'bg-pink-100 text-pink-700' };
            }
            if (/instagram\.com\/stories\//i.test(url)) {
                return { type: 'story', label: 'Instagram Story detected', color: 'bg-purple-100 text-purple-700' };
            }
            if (/instagram\.com\/tv\//i.test(url)) {
                return { type: 'video', label: 'IGTV Video detected', color: 'bg-purple-100 text-purple-700' };
            }
            if (/instagram\.com\/p\//i.test(url)) {
                return { type: 'post', label: 'Instagram Post detected', color: 'bg-orange-100 text-orange-700' };
            }
            return null;
        }
